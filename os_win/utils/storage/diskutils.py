...
class DiskUtils(baseutils.BaseUtils):
    ...
    def _get_disk_by_number(self, disk_number, msft_disk=True):
        if msft_disk:
    ...
    def _get_disks_by_unique_id(self, unique_id, unique_id_format,
        ...
        if not disks:
            err_msg = _("Could not find the disk having unique id "
    ...
    def get_device_name_by_device_number(self, device_number):
        disk = self._get_disk_by_number(device_number, msft_disk=False)
        return disk.Name
    ...
    def _parse_scsi_page_83(self, buff,
        ...
        if not vpd_page.PageLength:
            LOG.info(_('Page 0x83 data does not contain any '
    ...
    def _parse_scsi_id_desc(self, id_desc_addr, buff_sz):
        ...
        if id_desc.CodeSet == SCSI_ID_CODE_SET_ASCII:
            parsed_id = bytes(identifier).decode('ascii').strip('\x00')
        else:
            parsed_id = _utils.byte_array_to_hex_string(raw_id)
