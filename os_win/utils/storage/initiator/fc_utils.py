...
class FCUtils(object):
    ...
    def _get_persistent_binding(self, hba_handle):
        entry_count = 0
        hba_status = HBA_STATUS_ERROR_MORE_DATA

        while hba_status == HBA_STATUS_ERROR_MORE_DATA:
            binding = fc_struct.get_fc_binding_struct(entry_count)
            hba_status = self._run_and_check_output(
                hbaapi.HBA_GetFcpPersistentBinding,
                hba_handle,
                ctypes.byref(binding),
                ignored_error_codes=[HBA_STATUS_ERROR_MORE_DATA])
            entry_count = binding.NumberOfEntries
