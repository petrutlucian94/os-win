...
class HBA_FCPBindingEntry(ctypes.Structure):
    _fields_ = [('Type', HBA_FCPBindingType),
                ('ScsiId', HBA_ScsiId),
                ('FcpId', HBA_FcpId),
                ('FcId', ctypes.c_uint32)]


def get_fc_binding_struct(entry_count=0):
    class HBA_FCPBinding(ctypes.Structure):
        _fields_ = [('NumberOfEntries', ctypes.c_uint32),
                    ('Entries', HBA_FCPBindingEntry * entry_count)]

        def __init__(self, entry_count=0):
            self.NumberOfEntries = entry_count
            self.Entries = (HBA_FCPBindingEntry * entry_count)()
